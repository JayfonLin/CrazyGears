...
public class PhysicsEditorShapeLibrary {
    ...
    public Body createBody(String name, IShape pShape, PhysicsWorld pPhysicsWorld) {
		...
		bodyDef.position.x = sceneCenterCoordinates[Constants.VERTEX_INDEX_X] / this.pixelToMeterRatio;
		bodyDef.position.y = sceneCenterCoordinates[Constants.VERTEX_INDEX_Y] / this.pixelToMeterRatio;

		final Body body = pPhysicsWorld.createBody(bodyDef);

        for(FixtureTemplate fixtureTemplate : bodyTemplate.fixtureTemplates) {
            if (fixtureTemplate.type.equalsIgnoreCase("polygon")) {
            ...
            } else if (fixtureTemplate.type.equalsIgnoreCase("circle")) {
                CircleShape circleShape = new CircleShape();
                final FixtureDef fixtureDef = fixtureTemplate.fixtureDef;
                circleShape.setRadius(fixtureTemplate.circle.radius);
                //circleShape.setPosition(new Vector2(fixtureTemplate.circle.x, fixtureTemplate.circle.y));
                circleShape.setPosition(new Vector2(0/pixelToMeterRatio, 0/pixelToMeterRatio));
    ...
    protected static class ShapeLoader extends DefaultHandler {
        ...
        public void endElement(String uri, String localName, String name) throws SAXException {
            ...
            if (localName.equalsIgnoreCase(TAG_POLYGON)) {
                ...
                if (currentType.equalsIgnoreCase(TAG_POLYGON)) {
                    currentFixtures.get(currentFixtures.size()-1).setPolygons(currentPolygons);
                } else if (currentType.equalsIgnoreCase(TAG_CIRCLE)){
                    currentFixtures.get(currentFixtures.size()-1).setCircle(currentCircle);
                }
        ...
        public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
            ...
            if (localName.equalsIgnoreCase(TAG_BODY)) {
                ...
                currentType = attributes.getValue(TAG_TYPE);
                ...
                float radius = Float.parseFloat(attributes.getValue(TAG_R)) / this.pixelToMeterRatio;
                float x = Float.parseFloat(attributes.getValue(TAG_X)) / this.pixelToMeterRatio;
                float y = Float.parseFloat(attributes.getValue(TAG_Y)) / this.pixelToMeterRatio;
                currentCircle = new CircleTemplate(radius, x, y);
            }
        }
    }
    ...
    private static class FixtureTemplate {
        ...
        public void setCircle(CircleTemplate pCircleTemplate) {
            circle = pCircleTemplate;
        }
    }
    ...
    private static class CircleTemplate{
        ...
        public float x; 
        public float y;
        public CircleTemplate(float pRadius, float pX, float pY) {
            radius = pRadius;
            x = pX;
            y = pY;
        }
